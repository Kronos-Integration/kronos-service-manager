                ...
                if ((m = suffix.match(/^([^\/]+)\/([^\/]+)(\/(.+))?/))) {
                  ...
                  if (m[1] === 'steps') { // alias
                    ...
                    if (targetStep) {
                      ...
                      const newEndpoint = kronosStep.createEndpoint(endpointName, targetStep.endpoints[
                        targetEndpointName]);
                      console.log(  `substeps: ${endpointName} ${endpoint.target} ${JSON.stringify(targetEndpoint)}`);

                      step.addEndpoint(newEndpoint);
                      */
                    } else {
          ...
          switch (flow.state) {
            ...
            case 'paused':
              for (let sid in flow.steps) {
                ...
                try {
                  //step.meta.initialize.apply(step, manager, step);
