...
function createSteps(manager, definition, sr) {
  ...
  try {
    for (let stepName in definition) {
      ...
      if (!stepImpl) {
        sr.error('Implementation not found', 'step-type', { name: stepDef.type });
        continue;
      }
    ...
    for (let stepName in steps) {
                ...
                if ((m = suffix.match(/^([^\/]+)\/([^\/]+)(\/(.+))?/))) {
                  ...
                  if (m[1] === 'steps') { // alias
                    ...
                    if (targetStep) {
                      ...
                      sr.error('Unknown target step', 'step', {
                        name: targetStepName
                      });
                    }
                  } else {
      ...
      for (let endpointName in step.meta.endpoints) {
        ...
        if (metaEndpoint.mandatory && !step.endpoints[endpointName]) {
          sr.error('Mandatory endpoint not defined', 'endpoint', {
            name: endpointName
          });
        }
      }
