 ...
 * @return object with channels and steps
 */
		...
		for (let endpointName in step.endpoints) {
			...
			if (!endpoint.implementation) {
				...
				if (m) {
					...
					if (ei) {
						step.endpoints[endpointName] = endpointImpls.createEndpoint(endpointName,
							endpoint,
							ei.implementation(endpoint));
						/*
						...
						if (scheme === 'step') {
							if ((m = suffix.match(/^([^\/]+)\/([^\/]+)(\/(.+))?/))) {
								...
								if (m[1] === 'steps') { // alias
									...
									if (targetStep) {
										/*console.log(
											`endpoint alias ${step}/${endpoint} -> ${targetStep}/${targetEndpointName} ${targetStep.endpoints[targetEndpointName]}`
										);
                    */
										step.endpoints[endpointName] = endpointImpls.createEndpoint(endpointName, targetStep.endpoints[
											targetEndpointName]);
									} else {
									...
									if (targetStep) {
										...
										if (!targetEndpoint) {
											...
											targetStep.endpoints[targetEndpointName] = endpointImpls.createEndpoint(targetEndpointName,
												endpoint);
										}
