...
function createEndpoint(endpointMetaDefinition, endpointDefinition) {
  const properties = {};
  ...
  return Object.create(endpointMetaDefinition, properties);
}
  ...
  for (let stepName in definition) {
    ...
    if (stepDef.endpoints) {
      for (let endpointName in stepImpl.endpoints) {
        endpoints[endpointName] = createEndpoint(
      ...
      for (let endpointName in stepDef.endpoints) {
        ...
        let eDef = stepDef.endpoints[endpointName];
        ...
        endpoints[endpointName] = Object.create(endpointImpls.defaultEndpoint, properties);
      }
    }
            ...
            if (scheme === 'step') {
              if ((m = suffix.match(/^([^\/]+)\/([^\/]+)(\/(.+))?/))) {
                ...
                if (m[1] === 'steps') { // alias
                  ...
                  if (targetStep) {
                    console.log(`endpoint alias ${step}/${endpoint} -> ${targetStep}/${targetEndpointName}`);
                  } else {
