...
function createSteps(manager, definition, pr) {
  ...
  for (let stepName in definition) {
    ...
    if (stepDef.steps) {
      logger.debug(`Step : ${stepName} has sub steps`);
    ...
    if (stepDef.endpoints) {
      for (let endpointName in stepImpl.endpoints) {
        ...
        endpoints[endpointName] = endpointImplementation.createEndpoint(endpointName,
  ...
  for (let stepName in steps) {
    ...
    for (let endpointName in step.endpoints) {
      ...
      if (!endpoint.implementation) {
        logger.debug(`Endpoint '${endpointName}' of step '${stepName}' has no implementation`);
        ...
        if (m) {
          const scheme = m[1];
          const suffix = m[2];
          const ei = endpointImplementation.implementations[scheme];
          if (ei) {
            step.addEndpoint(endpointImplementation.createEndpoint(endpointName, ei.implementation, endpoint));
          } else {
            if (scheme === 'step') {
              if ((m = suffix.match(/^([^\/]+)\/([^\/]+)(\/(.+))?/))) {
                ...
                if (m[1] === 'steps') { // alias
                  ...
                  if (targetStep) {
                    step.addEndpoint(endpointImplementation.createEndpoint(endpointName, targetStep.endpoints[
                      targetEndpointName]));
                  } else {
                  ...
                  if (targetStep) {
                    ...
                    if (!targetEndpoint) {
                      ...
                      targetStep.addEndpoint(endpointImplementation.createEndpoint(targetEndpointName,
                        endpoint));
                    }
    ...
    for (let endpointName in step.meta.endpoints) {
      ...
      if (metaEndpoint.mandatory && !step.endpoints[endpointName]) {
        logger.error(`Mandatory ${endpointName} not defined`);
