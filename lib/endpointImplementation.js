...
const defaultEndpoint = {
  toString() {
    ...
    get initialize() {
      return function (manager, step) {
        const generatorFunction = this.implementation.apply(this);

        //console.log(`A ${step.name}/${this.name} ${JSON.stringify(this)} ${generatorFunction}`);

        // TODO: why is sometimes the funcation alredy the generatorObject ?
        const generatorObject = util.isFunction(generatorFunction) ? generatorFunction() : generatorFunction;

        console.log(`initialize ${step.name}/${this.name} -> ${generatorObject}`);
