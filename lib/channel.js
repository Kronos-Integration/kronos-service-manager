...
exports.create = function (step1, ep1, step2, ep2) {
  ...
  exp.expand({ someKey: "aValue", name: "${fileName}"});
	...
	if (ep1.isOutAndCanBeActive && ep2.isInAndCanBePassive) {
		...
		imp2 = function (manager, genertor) {
          ...
          // ep2.contentInfo
          request.info = exp.expand({ someKey: "aValue", name: "${fileName}"});
	...
	return Object.create(RootChannel, {
		...
		endpointA: {
			value: endpointImplementation.createEndpoint(ep1.name, {
				description: `Channel ${name}`,
				implementation: imp1
			}, ep1)
		},
		endpointB: {
			value: endpointImplementation.createEndpoint(ep2.name, {
				description: `Channel ${name}`,
				implementation: imp2
			}, ep2)
		}
