...
exports.create = function (step1, ep1, step2, ep2) {
	...
	if (ep1.isOutAndCanBeActive && ep2.isInAndCanBePassive) {
		...
		imp1 = function (manager) {
			logger.debug(`Imp1: Endpoint Set '${name}'`);

			const myGen = function* () {
				logger.debug(`Imp1: Initialize iterator '${name}'`);
				let iterator = generatorObject();
				iterator.next();
				logger.debug(`Imp1: Iterator initialized '${name}'`);

				while (true) {
					logger.debug(`Imp1: Generator called '${name}'`);
					...
					logger.debug(`Imp1: Transport the request to next. '${name}'`);
			...
			logger.debug(`Imp1: Generator created '${name}'`);
			let iterator = myGen();
			//iterator.next();
			return iterator;
		};
		...
		imp2 = function (manager, aGeneratorFunction) {
			...
			logger.debug(`Imp2: Endpoint Set '${name}'`);
