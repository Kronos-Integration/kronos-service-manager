	...
	if (ep1.isOutAndCanBeActive && ep2.isInAndCanBePassive) {
		...
		imp1 = function (manager) {
			...
			const myGen = function* () {
				logger.debug(`Imp1: Initialize iterator`);
				let iterator = generatorObject();
				iterator.next();
				logger.debug(`Imp1: Iterator initialized`);
				...
				while (true) {
					...
					logger.debug(`Imp1: Transport the request to next.`);
			...
			logger.debug(`Imp1: Generator created`);
			let iterator = myGen();
			//iterator.next();
			return iterator;
		};
