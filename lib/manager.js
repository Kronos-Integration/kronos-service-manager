 ...
 *    npmModulesNames - npm module names to register
...
exports.manager = function (options) {
  ...
  llm.defineLoggerMethods(manager, llm.defaultLogLevels);
  llm.defineLogLevelProperties(manager, llm.defaultLogLevels, llm.defaultLogLevels.info);
    ...
    if (typeof serviceOrServiceName === 'string') {
      ...
      */
      ...
      if (values.name) {
        ...
        if (baseService === null) {
          this.error(level => `Unable to find base service: ${values.name}`);
        }
  ...
  manager.registerFlow = function (newFlow) {
    if (flows[newFlow.name]) {
      ...
      if (newFlow.autostart) {
        return newFlow.start();
      }
      return Promise.resolve(newFlow);
    }
  };
  ...
  manager.registerModules = function (moduleNames) {
    try {
        ...
        name => {
          try {
            ...
            if (m) {
              ...
              this.error(level => `unable to register module ${name}`);
            }
          } catch (e) {
            this.error(level => `unable to register module ${name}`);
          }
      ...
      this.error(level => `unable to register modules`);
    }
  };

  if (options.npmModulesNames) {
    manager.registerModules(options.npmModulesNames);
  }
