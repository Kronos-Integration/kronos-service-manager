...
// Erst mal rausgenommen damit die streams wieder funktionieren.
...
exports.manager = function (options) {
  ...
  manager.stopFlow = function (flowName) {
    ...
    return new Promise(function (resolve, reject) {
      ...
      if (!flow) {
        reject(Error('No such flow'));
      } else {
        // TODO stop all channels/endpoints
        //flow.stop();
        resolve(flow);
      }
    });
  ...
  manager.deleteFlow = function (flowName) {
    return manager.stopFlow(flowName).then(function (flow) {
      delete flowDefinitions[flowName];
      // TODO unload channels ...

      //console.log(`Delete flow: ${flowName} ${flow} -> ${flowDefinitions[flowName]}`);
