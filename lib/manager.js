...
exports.manager = function (options) {
   ...
   */
  manager.registerFlows = function (flowDef, progressReporter) {
    if (!progressReporter) {
      progressReporter = options.progressReporter;
    }

    function _declare() {
      const newFlows = flow.create(manager, flowDef, progressReporter);

      const toBeReplaced = [];

      newFlows.forEach(function(f) {
        const oldFlow = flowDefinitions[f.name];
        if(oldFlow) { toBeReplaced.push(oldFlow.delete()); }
  ...
  return new Promise(function (resolve, reject) {
    ...
    return uti.initialize({
      ...
      .then(function (resloved) {
        if (options.flows) {
          manager.registerFlows(options.flows, options.progressReporter);
        }
