...
exports.manager = function (options) {
  ...
  manager.shutdown = function () {
    return Promise.all(Object.keys(flows).map(name => flows[name].stop())).then(Promise.resolve(manager));
  };
   ...
   */
  manager.registerFlow = function (newFlow, scopeReporter) {
    if (!scopeReporter) {
   ...
   * @return returns a promise that is fullfilled when the flow is removed
   */
